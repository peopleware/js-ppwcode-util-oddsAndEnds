define(["dojo/_base/declare", "dijit/_WidgetBase", "ppwcode-util-contracts/_Mixin",
  ...
  function(declare, _WidgetBase, _ContractMixin,
    ...
    var AbstractDraggablePane = declare([_ContractMixin], {
      ...
      // description:
      //   DraggablePanes exposes an interface to the HorizontalPanesContainer and peer instances, not to be used by
      // others, by which they behave as elements in a double-linked-list with sentinels at both ends. During the drag
      // of a pane, it is removed from the double-linked-list, and inserted again on drop.  This type is explicitly not
      // Stateful (or a _WidgetBase). External users should not be concerned with the instance variables.  Since there
      // is only 1 mouse, in any application, there can only be 1 Pane "on the move". "Move" animations however could
      // play concurrently in more than 1 instance of HorizontalPanesContainer. They cannot be stored "statically".
    ...
    var DraggablePane = declare([_WidgetBase, HasNext, HasPrevious], {
      ...
      //   DraggablePanes exposes an interface to the HorizontalPanesContainer and peer instances, not to be used by
      // others, by which they behave as elements in a double-linked-list with sentinels at both ends. During the drag
      // of a pane, it is removed from the double-linked-list, and inserted again on drop.
      ...
      _focusChanged: function(oldFocusStack, newFocusStack) {
        if (((!oldFocusStack || oldFocusStack.indexOf(this.id) < 0) &&
             (newFocusStack && newFocusStack.indexOf(this.id) >= 0)) || // had no focus, and now has focus
            ((!oldFocusStack || oldFocusStack.indexOf(this.id) >= 0) &&
             (newFocusStack && newFocusStack.indexOf(this.id) < 0))) { // had focus, and has no longer
          return this;
        }
        else {
          return this.next._focusChanged(oldFocusStack, newFocusStack);
      ...
      _getTargetRightPosition: function(/*Number?*/ referencePosition) {
        ...
        return this._getTargetLeftPosition(referencePosition) + this._getWidth() +
               ((this.get("focused") && this.next !== this.getLast()) ? extraGapForFocused : 0);
      },
      ...
      _getTargetLeftPosition: function(/*Number?*/ referencePosition) {
        ...
        if ((!referencePosition || referencePosition !== 0) && this.get("focused") &&
